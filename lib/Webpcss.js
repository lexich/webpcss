...
class Webpcss {
  ...
  asyncProcessNode(decl) {
    ...
    const rx = options.replace_from instanceof RegExp
      ...
      : new RegExp(options.replace_from, "g");
    const asyncUrls = urls.map((item)=> {
      ...
      if (item.mimetype === "url") {
        ...
        if (!options.inline) {
          ...
          if (shouldAddClass) {
            src = isFunction(replaceTo) ? replaceTo({
              url,
          ...
          if (shouldAddClass) {
            ...
            return readFileAsync(urlPath)
              .then((data)=> base64.convert(data, options.cwebp_configurator)
                .then(
                  (buffer)=> buffer && `url(data:image/webp;base64,${buffer.toString("base64")})`
                )
                .catch(()=> `url(${item.data})`))
              .catch(()=> {
                breaks+= 1;
