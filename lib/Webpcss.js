  ...
  asyncProcessNode(decl) {
    ...
    return Promise.all(asyncUrls).then((urls)=> {
      if (breaks !== urls.length) {
        // add .no-webp
        const selectorNoWebP = decl.parent.selectors.map(
          (sel)=> options.process_selector(sel, options.no_webp)
        ).join(", ");

        const noWebpRule = Webpcss.formatRule(
          decl.parent.cloneBefore({ selector: selectorNoWebP }));
        ...
        return Webpcss.appendToCopyTree(decl.parent.parent, webpRule);
      }
    });
  }
